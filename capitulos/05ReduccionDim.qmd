...
```{python}
#| echo: false
perf_f = Markdown(f'{perf[27]:0.4f}')
```

La @fig-rendimiento-sel muestra el rendimiento de las 30 variables, se observa como una gran parte de las variables proporcionan un rendimiento superior al $0.8$ y la variable que tiene el mejor rendimiento es la que corresponde al índice $27$ y valor `{python} perf_f`.

```{python}
#| fig-cap: Rendimiento de las Variables
