...
Con el objetivo de entender el funcionamiento del Teorema de Bayes, esta sección presenta un problema sintético. El procedimiento es el siguiente, primero se generarán los datos, los cuales van a ser tres nubes de puntos generadas mediante tres distribuciones gaussianas multivariadas. Con estas tres nubes de puntos, se utilizará el Teorema de Bayes (@eq-teorema-bayes) para clasificar todos los puntos generados. 
...
Una vez definidas las distribuciones podemos generar números aleatorios de las mismas, en el siguiente código se generar 1000 vectores aleatorios de las tres distribuciones. 
...
Generar una nube de puntos similar a la que se muestra en la @fig-guasiana-alargada-x. Observar que ambos ejes de la figura están en el rango $[-20, 20].$

```{python} 
#| label: fig-guasiana-alargada-x
#| fig-cap: "Distribución Gausiana"
#| echo: false
ejemplo = multivariate_normal(mean=[5, 5],
                              cov=[[20, -5], [-5, 2]])
X_e = ejemplo.rvs(size=1000)
df = pd.DataFrame(X_e, columns=['x', 'y'])
...
En esta sección se describe el primer ejemplo del paso 4 de la metodología general (ver @sec-metodologia-general) de los algoritmos de aprendizaje supervisado. El algoritmo $f$ mencionado en la metodología corresponde en este ejemplo al uso del Teorema de Bayes y las distribuciones `p1`, `p2` y `p3` y los correspondientes valores a priori. 
...
El error estándar (ver @sec-error-estandar) está definido como $\sqrt{\mathbb V(\hat \theta)}$ donde $\hat \theta$ es el valor estimado, en este caso el `error`. El error es una variable aleatoria que sigue una distribución de Bernoulli, dado que para cada ejemplo tiene dos valores $1$ que indica que en ese ejemplo el clasificador se equivocó y $0$ cuando se predice la clase correcta. El parámetro de la distribución Bernoulli, $p$, se estima como la media entonces el error estándar de $p$ corresponde al error estándar de la media (ver @sec-error-estandar-media), i.e., $\sqrt{\mathbb V(\hat p)} = \sqrt{\frac{\hat p (1 - \hat p)}{N}},$ dado que la varianza $\sigma^2$ de una distribución Bernoulli con parámetro $p$ es $p (1 - p)$. Para el ejemplo analizado el error estándar se calcula con la siguiente instrucción; teniendo un valor de `{python} se_formula_f`.
...
El error estándar corresponde a la distribución que tiene la estimación del parámetro de interés; mediante la técnica de muestreo boostrap se simuló está distribución y con el siguiente código se puede observar su histograma, donde los datos estimados se encuentran en la lista `B`.
...
## Error Bayesiano {#sec-error-bayesiano}

El ejemplo anterior deja una pregunta abierta, ¿por qué un modelo que conoce exactamente las distribuciones que generaron los datos (`p1`, `p2` y `p3`) comete errores al clasificar? La respuesta está en que el error no depende únicamente del método utilizado para clasificar, sino también de la superposición entre las distribuciones de las clases; esta sección formaliza esta idea mediante el concepto de **error bayesiano** y muestra que la regla utilizada en @sec-prediccion-normal, es decir, seleccionar la clase con mayor probabilidad *a posteriori*, es la que menor error comete entre todos los clasificadores posibles.

Sea $f: \mathcal X \rightarrow \mathcal Y$ un clasificador, es decir, una función que asigna una clase $f(x)$ a cada entrada $x$; por ejemplo, la variable `prediccion` del ejemplo anterior corresponde a $f(x)$ evaluado en cada uno de los elementos de `X`. La probabilidad de que $f$ se equivoque, condicionada a una entrada $\mathcal X=x$, es

$$
\mathbb P(f(\mathcal X) \neq \mathcal Y \mid \mathcal X=x) = 1 - \mathbb P(\mathcal Y = f(x) \mid \mathcal X=x),
$$ {#eq-error-condicional}

dado que $f(x)$ únicamente puede tomar un valor y el complemento de acertar es equivocarse. El error del clasificador $f$, sin condicionar a una entrada particular, se obtiene promediando sobre todas las posibles entradas, es decir, $\mathbb P(f(\mathcal X) \neq \mathcal Y) = \mathbb E_{\mathcal X}\left[\mathbb P(f(\mathcal X) \neq \mathcal Y \mid \mathcal X)\right].$

El clasificador utilizado en @sec-prediccion-normal corresponde a la regla

$$
f^*(x) = \underset{k}{\text{arg max }} \mathbb P(\mathcal Y=k \mid \mathcal X=x),
$$ {#eq-clasificador-bayes}

conocida como la **regla de Bayes**, y al error que esta regla produce, $\mathbb P(f^*(\mathcal X) \neq \mathcal Y)$, se le conoce como **error bayesiano**. Usando la @eq-error-condicional en $f^*$ se tiene que

$$
\mathbb P(f^*(\mathcal X) \neq \mathcal Y \mid \mathcal X=x) = 1 - \underset{k}{\max}\ \mathbb P(\mathcal Y=k \mid \mathcal X=x).
$$ {#eq-error-bayesiano}

El error bayesiano no depende del algoritmo utilizado sino de qué tan traslapadas están las clases; en el ejemplo de @sec-tres-normales el valor `{python} error_f` estimado no es producto de un método imperfecto -- `p1`, `p2` y `p3` son las distribuciones exactas -- sino que corresponde, salvo el error de estimación medido con `se_f`, al error bayesiano del problema, es decir, es un error irreducible.

Falta mostrar que $f^*$ es, de entre todos los clasificadores posibles, el que tiene el menor error esperado. Sea $f$ cualquier otro clasificador y, para simplificar la notación, sea $p_k(x) = \mathbb P(\mathcal Y=k \mid \mathcal X=x)$. Por definición de máximo, para toda entrada $x$ se cumple que

$$
p_{f(x)}(x) \leq \underset{k}{\max}\ p_k(x),
$$ {#eq-paso-max}

dado que el lado derecho es el mayor valor que puede tomar $p_k(x)$ mientras que el lado izquierdo es solo uno de esos valores, el correspondiente a la clase $f(x)$ que selecciona el clasificador $f$.

El siguiente paso es multiplicar ambos lados de la @eq-paso-max por $-1$, operación que invierte el sentido de la desigualdad, y después sumar $1$ a ambos lados, lo cual no altera el sentido de la desigualdad al sumarse la misma constante en ambos lados:

$$
\begin{split}
p_{f(x)}(x) &\leq \underset{k}{\max}\ p_k(x) \\
-p_{f(x)}(x) &\geq -\underset{k}{\max}\ p_k(x) \\
1 - p_{f(x)}(x) &\geq 1 - \underset{k}{\max}\ p_k(x).
\end{split}
$$

Finalmente se identifica cada lado de esta última desigualdad, el lado izquierdo, $1-p_{f(x)}(x) = 1 - \mathbb P(\mathcal Y=f(x) \mid \mathcal X=x)$, corresponde a $\mathbb P(f(\mathcal X) \neq \mathcal Y \mid \mathcal X=x)$ por la @eq-error-condicional; el lado derecho, $1 - \underset{k}{\max}\ p_k(x)$, corresponde a $\mathbb P(f^*(\mathcal X) \neq \mathcal Y \mid \mathcal X=x)$ por la @eq-error-bayesiano. Sustituyendo ambas identidades se obtiene

$$
\mathbb P(f(\mathcal X) \neq \mathcal Y \mid \mathcal X=x) \geq \mathbb P(f^*(\mathcal X) \neq \mathcal Y \mid \mathcal X=x) \quad \text{para toda } x.
$$

Como esta desigualdad se cumple para cada valor de $x$, se preserva al tomar el valor esperado sobre $\mathcal X$ en ambos lados, por lo tanto

$$
\mathbb P(f(\mathcal X) \neq \mathcal Y) \geq \mathbb P(f^*(\mathcal X) \neq \mathcal Y) \quad \text{para todo clasificador } f,
$$

es decir, ningún clasificador puede tener un error esperado menor al de la regla de Bayes; esta propiedad se conoce como la **optimalidad del clasificador Bayesiano**. En la siguiente sección se retoma esta idea desde la perspectiva del riesgo, donde se observará que minimizar el riesgo bajo un costo $0/1$ es equivalente a minimizar el error aquí definido.
...
La @fig-2-guasiana-alargada-x muestra el conjunto de datos donde se realizará el ejemplo de seleccionar la clase considerando el riesgo. 


```{python} 
#| label: fig-2-guasiana-alargada-x
#| fig-cap: "Dos distribución Gausianas asociadas a dos clases"
#| code-fold: true
#| echo: true
...
```{python} 
#| label: fig-2-guasiana-original-reducido
#| fig-cap: "Predicción del conjunto original y el conjunto que elimina aquellos elementos que se asocian a la clase nula"
#| code-fold: true
#| echo: true
...
La actividad corresponde en calcular el error y el número de elementos que no están asociados a la clase nula cuando $\lambda = 0.2$. La @fig-2-guasiana-alargada-actividad muestra los datos generados y los datos válidos considerando el nivel de riesgo solicitado así como los errores en la predicción. 

```{python} 
#| label: fig-2-guasiana-alargada-actividad
#| fig-cap: "Predicción del conjunto original y el conjunto que elimina aquellos elementos que se asocian a la clase nula"
#| echo: false
...
p1 = multivariate_normal(mean=[5, 5],
                              cov=[[20, -5],
                                   [-5, 2]])
p2 = multivariate_normal(mean=[5, 5],
                              cov=[[20, 5],
                                   [5, 2]])
X1 = p1.rvs(size=1000)
X2 = p2.rvs(size=1000)
...
hy = np.array([m[i]
               for i in posterior.argmax(axis=1)])
error1 = f'{(y != hy).mean():0.4f}'

riesgo = 0.20
valid = posterior.max(axis=1) > 1 - riesgo
error2 = f'{(y[valid] != hy[valid]).mean(): 0.4f}'
