...
if naive > completo:
    difference = lambda s: (y_g[s] != hy_naive[s]).mean() -  (y_g[s] != hy_gaussian[s]).mean()
else:
    difference = lambda s: (y_g[s] != hy_gaussian[s]).mean() - (y_g[s] != hy_naive[s]).mean()
B = [difference(s) for s in S]
se = np.std(B, axis=0)
```
...
```{python}
w_f = ', '.join([f'{v:0.2f}' for v in m.coef_])
w_f = Markdown(f'$\\mathbf w=[{w_f}]$')
w_0 = Markdown(f'$w_0={m.intercept_:0.2f}$')
```
Los coeficientes de la regresión lineal son `{python} w_f` y `{python} w_0` lo cual se encuentran en las siguientes variables
...
```{python}
#| echo: false
se_f = ', '.join([f'{v:0.2f}' for v in w / se])
se_f = Markdown(f'$[{se_f}]$')
```

y para saber si los coeficientes son significativamente diferente de cero se calcula el cociente `m.coef_` entre `se`; teniendo los siguientes valores `{python} se_f`. Se observa que hay varios coeficientes con valor absoluto menor que 2, lo cual significa que esas variables tiene un coeficiente que estadísticamente no es diferente de cero. 
