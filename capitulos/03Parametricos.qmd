...
from CompStats.metrics import accuracy_score, mean_squared_error
```
...
### Distribución de Bernoulli {#sec-distribuccion-de-bernoulli}
...
Habiendo descrito el proceso para estimar los parámetros de una distribución, por un lado se presentó de manera teórica con la distribución Bernoulli (ver @sec-distribuccion-de-bernoulli) y de manera práctica con una distribución Gaussiana (ver @sec-estimacion-distribucion-gausiana), se está en la posición de usar todos estos elementos para presentar el proceso completo de clasificación. La metodología general de aprendizaje supervisado (ver @sec-metodologia-general) está definida por cinco pasos, estos pasos se especializan para el problema de clasificación y regresión, utilizando modelos paramétricos, de la siguiente manera. 
...
Siguiendo los pasos en estimación de parámetros de una Gaussiana (@sec-estimacion-distribucion-gausiana) se pueden estimar los parámetros para cada Gaussiana dada la clase. Es decir, se tiene que estimar los parámetros $\mu$ y $\Sigma$ para la clase $1$, $2$ y $3.$ El algoritmo de clasificación que estima $\mu$ y $\Sigma$ para cada clase se le conoce como **Analizador Discriminante Cuadrático** implementando en la clase `QuadraticDiscriminantAnalysis`.

Una implementación directa para la estimación de los parámetros se puede realizar iterando por las etiquetas contenidas en la variable `labels` y seleccionando los datos en `T` que corresponden a la clase analizada, ver el uso de la variable `mask` en la línea 4 y 5. Después se inicializa una instancia de la clase `multivariate_normal` para ser utilizada en el cómputo de la función de densidad de probabilidad. El paso final es guardar las instancias de las distribuciones en la lista `likelihood`.
...
Estas estimaciones se pueden comparar con los parámetros reales (@sec-tres-normales). También se puede calcular su error estándar para identificar si el parámetro real, $\theta$, se encuentra en el intervalo definido por $\hat \theta - 2\hat{se} \leq \hat \theta \leq \hat \theta + 2 \hat{se}$ que corresponde aproximadamente al 95% de confianza asumiendo que la distribución de la estimación del parámetro es Gaussiana.
...
Cuando se asume que las matrices de covarianza son iguales para todas las clases, se está en la presencia del algoritmo **Analizador Discriminante Lineal**. Este algoritmo está implementado en la clase `LinearDiscriminantAnalysis`.

::: {.callout-note}

La clase `LinearDiscriminantAnalysis` se utiliza con la siguiente instrucción, el parámetro `store_covariance` se asigna para poder mostrar la matriz de covarianza. 

```{python}
#| echo: true

lda = LinearDiscriminantAnalysis(store_covariance=True).fit(T, y_t)
```

Los parámetros $\mu$ se encuentran en el siguiente atributo. Se puede observar que los valores estimados por el método explicado son los encontrados anteriormente. 
...
Los parámetros estimados para la matriz de covarianza, $\Sigma$, se pueden obtener en el siguiente atributo
```{python}
#| echo: true
...
De manera similar la predicción del algoritmo `LinearDiscriminantAnalysis` se realiza con la siguiente instrucción. 

```{python}
#| echo: true

lda_hy = lda.predict(G)
```
:::
...
En el clasificador Bayesiano Ingenuo se asume que las variables $\mathcal X_i$ y $\mathcal X_j$ para $i \neq j$ son independientes, esto trae como consecuencia que $f(\mathcal X_1, \mathcal X_2, \ldots, \mathcal X_d) = \prod_i^d f(\mathcal X_i).$ Esto quiere decir que cada variable está definida como una Gaussiana donde se tiene que identificar $\mu$ y $\sigma^2.$
...
Esta sección ilustra el uso del clasificador Bayesiano al generar dos modelos; Clasificador Bayesiano (mejor conocido como el algoritmo de Análisis Discriminante Cuadrático) y Bayesiano Ingenuo en el conjunto de datos de *Breast Cancer Wisconsin.* Estos datos se pueden obtener utilizando la función `load_breast_cancer` tal y como se muestra a continuación.
...
```{python}
#| echo: true
qda = QuadraticDiscriminantAnalysis(reg_param=1e-3).fit(T, y_t)
naive = GaussianNB().fit(T, y_t)
```
...
::: {.callout-note}
El error es el complemento de la exactitud (@sec-accuracy), utilizando la librería [CompStats](https://compstats.readthedocs.io/en/latest/metrics_api.html) (@Nava2024), en particular la función que implementa la exactitud, se puede medir el rendimiento de los dos algoritmos tal y como se muestra en el siguiente código. En la primera linea se mide el rendimiento de QDA y en la segunda el rendimiento que corresponde a Bayesiano Ingenuo. La última linea gráfica el rendimiento. 

```{python}
#| echo: true
#| fig-cap: Rendimiento entre QDA y Bayesiano Ingenuo.
#| label: fig-redimiento-acc

score = accuracy_score(y_g, hy_qda, name='QDA')
score(hy_naive, name='Bayesiano Ingenuo')
score.plot(comp_legend='Comparación',
           winner_legend='Mejor',
...
En la @fig-redimiento-acc se observa el rendimiento, con su intervalo de confianza al 95%. También se incluye, el resultado de comparar estadísticamente (95%) la diferencia de rendimiento mediante el color. 
...
Incluir en la comparación anterior el algoritmo Analizador Discriminante Lineal, tal y como se muestra en la @fig-redimiento-acc-act. 

```{python}
#| echo: false
#| fig-cap: Rendimiento entre QDA y Bayesiano Ingenuo.
#| label: fig-redimiento-acc-act

score(LinearDiscriminantAnalysis().fit(T, y_t).predict(G),
      name='LDA')
score.plot(comp_legend='Comparación',
           winner_legend='Mejor',
           tie_legend='Equivalente',
...
T2 = np.concatenate((T[:, mask], T[:, mask]**2), axis=1)
G2 = np.concatenate((G[:, mask], G[:, mask]**2), axis=1)
```
...
```{python}
#| echo: true
m2 = LinearRegression().fit(T2, y_t)
hy2 = m2.predict(G2)
```
...
Utilizando la librería `CompStats` se puede hacer una comparación del rendimiento de la regresión logística utilizando todas las variables y solo aquellas que son estadísticamente significantes, tal y como se muestra en la @fig-rendimiento-ols-compstats

```{python}
#| echo: true
#| fig-cap: Rendimiento entre modelo con variables originales y selección de variables.
#| label: fig-rendimiento-ols-compstats

mse = mean_squared_error(y_g, hy, name='Completo')
mse(hy2, name='Reducido')
mse.plot(comp_legend='Comparación',
         winner_legend='Mejor',
         tie_legend='Equivalente',
         loser_legend='Diferente',
         alg_legend='Algoritmo',
...
Otra manera de ver el problema de regresión es como un problema de optimización, tal y como se muestra en @eq-min-l2; cabe resaltar que el problema mostrado en @eq-min-l2 tiene una solución analítica y no es necesario utilizar un sistema de optimización. 

$$
\min \mid\mid \mathbf y - (X \mathbf w + w_0) \mid\mid_2 
$$ {#eq-min-l2}

donde $X$ corresponde a las observaciones en el conjunto de entrenamiento y $\mathbf y$ es el valor medido; con esta información y la librería [CVXPY](https://www.cvxpy.org/index.html) (@cvxpy), el problema de la @eq-min-l2 se puede codificar con los siguientes pasos. En primer lugar se crean las variables del problema de optimización, i.e., $\mathbf w$ y $w_0$.
...
La función objetivo se genera, con la función `cp.norm` como se muestra a continuación

```{python}
#| echo: true
norm = cp.norm(y_t - (T@w + w0), 2)
```

Las últimas instrucciones corresponden a indicar que se está desea minimizar la función objetivo, construir el problema de optimización e introducir el problema al optimizador tal y como se observa en el siguiente código. 
...
Habiendo descrito el uso de la norma $L_2$ la cual tiene una solución analítica es cuestión de cambiar el parámetro en la función `cp.norm` para estimar los coeficientes $\mathbf w$ y $w_0$ usando la norma $L_1$, como se muestra en la siguientes instrucciones. 


```{python}
#| echo: true
norm = cp.norm(y_t - (T@w + w0), 1)
obj = cp.Minimize(norm)
prob = cp.Problem(obj)
valor = prob.solve()
```

La @fig-rendimiento-ols-l1-l2 muestra el rendimiento del uso de la norma $L_2$ (Completo), la norma $L_1$ y el modelo que se obtiene quitando aquellos elementos que son estadísticamente equivalentes a cero. 

```{python}
#| echo: true
#| fig-cap: Rendimiento entre modelo con variables originales, con la norma $L_1$ y selección de variables.
#| label: fig-rendimiento-ols-l1-l2

mse(G @ w.value + w0.value, name='L1')
mse.plot(comp_legend='Comparación',
         winner_legend='Mejor',
         tie_legend='Equivalente',
         loser_legend='Diferente',
...
Tener el problema de regresión como un problema de optimización, permite incluir de manera eficiente restricciones que pueden mejorar el rendimiento del regresor. Por ejemplo se podría restringir la norma de los coeficientes i.e., $\mathbf w$ para que sea menor que un parámetros, tal y como se muestra en la @eq-min-restricciones donde la norma $L_1$ de los coeficientes debe de ser menor al parámetro $\gamma$. 

$$
\begin{split}
\min & \mid\mid \mathbf y - (X \mathbf w + w_0) \mid\mid_2 \\
\text{sujeto a} & \\
& \mid \mid \mathbf w \mid \mid_1 \leq \gamma \\
\end{split}
$$ {#eq-min-restricciones}

El siguiente código muestra las instrucciones para codificar y resolver el problema descrito en la @eq-min-restricciones utilizando [CVXPY](https://www.cvxpy.org/index.html) (@cvxpy). La primera instrucción genera el parámetro $\gamma$ con el valor $1500$ (segunda linea), el resto de las lineas se han visto previamente, la única diferencia es la restricción $\mid\mid \mathbf w \mid\mid_1 \leq \gamma$ que se observa en la penúltima linea. 
...
norm = cp.norm(y_t - (T@w + w0), 2)
obj = cp.Minimize(norm)
prob = cp.Problem(obj, [cp.norm(w, 1) <= gamma])
value = prob.solve()
```

Se decidió utilizar la norma $L_1$ en la restricción porque esa norma hace que algunos de los coeficientes estimados sean cero o muy cercanos a cero, por ejemplo con el valor de $\gamma$ seleccionado se tienen `{python} (np.fabs(w.value)<1e-5).sum()` coeficientes menores que $10^{-5}$. En el siguiente código, la primera linea reemplaza los coeficientes cercanos a cero por ese valor y se genera la figura que presenta el rendimiento de los sistemas evaluados.

```{python}
#| echo: true
#| fig-cap: Rendimiento entre diferentes modelos.
#| label: fig-rendimiento-ols-l1-l2-gamma 
w.value[np.fabs(w.value)<1e-5] = 0
mse(G @ w.value + w0.value, name=f'gamma').plot(
...
En la @fig-rendimiento-ols-l1-l2-gamma se observa que el mejor rendimiento se obtiene con el sistema donde los coeficientes son seleccionados con la restricción de la norma $L_1$, aunque cabe mencionar que todos los sistemas son estadísticamente equivalentes entre si. 
