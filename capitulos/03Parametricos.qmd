...
En la @fig-redimiento-acc se observa el rendimiento, con su intervalo de confianza al 95%. También se incluye, el resultado de comparar estadisticamente (95%) la diferencia de rendimiento mediante el color. 
...
Utilizando la librería `CompStats` se puede hacer una comparación del rendimiento de la regresión logística utilizando todas las variables y solo aquellas que son estadísticamente significantes, tal y como se muetra en la @fig-rendimiento-ols-compstats
...
Otra manera de ver el problema de regresión es como un problema de optimización, tal y como se muestra en @eq-min-l2; cabe resaltar que el problema mostrado en @eq-min-l2 tiene una solución análita y no es necesario utilizar un sistema de optimización. 
...
Las últimas instrucciones corresponden a indicar que se está desea minimizar la función objetivo, construir el problema de optimización y pasarle el problema al optimizador tal y como se observa en el siguiente código. 
...
Habiendo descrito el uso de la norma $L_2$ la cual tiene una solución análita es cuestión de cambiar el parámetro en la función `cp.norm` para estimar los coeficientes $\mathbf w$ y $w_0$ usando la norma $L_1$, como se muestra en la siguientes instrucciones. 
...
La @fig-rendimiento-ols-l1-l2 muestra el rendimiento del uso de la norma $L_2$ (Completo), la norma $L_1$ y el modelo que se obtiene quitando aquellos elementos que son estadisticamente equivalentes a cero. 
...
En la @fig-rendimiento-ols-l1-l2-gamma se observa que el mejor rendimiento se obtiene con el sistema donde los coeficientes son seleccionados con la restricción de la norma $L_1$, aunque cabe mencionar que todos los sitemas son estadisticamente equivalentes entre si. 
