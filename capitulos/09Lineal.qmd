...
En lugar de utilizar directamente las características $\mathbf x$ se puede realizar una transformación $\phi$ a estas características, esto haría que el hiperplano (@eq-hiperplano) quede cómo $\mathbf w = \sum_i \alpha_i y_i \phi(\mathbf x_i)$, pasando esto a la función discrimintante $g$ queda como $g(\mathbf x) = \sum_i \alpha_i y_i \phi(\mathbf x_i) \cdot \phi(x) + w_0.$ El producto punto se puede cambiar por una función **Kernel**, i.e., $K(\mathbf x_i, \mathbf x_j) = \phi(\mathbf x_i) \cdot \phi(\mathbf x_j)$ lo cual hace que innecesaria la transformación al espacio $\phi.$ 
