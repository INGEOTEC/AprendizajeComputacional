...
Otra característica de $E(\mathbf w, w_0 \mid \mathcal D)$ es que no tiene una solución cerrada y por lo tanto es necesario utilizar un método de optimización para encontrar los parámetros $\mathbf w$ y $w_0$. 
...
T, G, y_t, y_g = train_test_split(D, y, test_size=0.4, random_state=3)
```
...
headers = 'Clasificador | Precision | Recall | $F_1$   |'
linea   = '-------------|-----------|--------|---------|'
