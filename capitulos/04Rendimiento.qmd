...
def grafica():
    ...
    for tr, vs in skf.split(X, y):
        gaussian = QuadraticDiscriminantAnalysis(reg_param=1e-3).fit(X[tr], y[tr])
...
$$
R^2(\mathcal Y, \mathcal{\hat Y}) = 1 - \frac{\sum_{i=1}^N (\mathcal Y_i - \mathcal{\hat Y}_i)^2)}{\sum_{i=1}^N (\mathcal Y_i - \mathcal{\bar Y})^2)}.
$$ {#eq-r2}
...
```{python}
#| echo: true
gaussian = QuadraticDiscriminantAnalysis(reg_param=1e-3).fit(np.concatenate((T, V)),
...
for ts, vs in kfold.split(T):
    gaussian = QuadraticDiscriminantAnalysis(reg_param=1e-3).fit(T[ts], y_t[ts])
