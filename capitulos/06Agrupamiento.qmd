...
Calcular $\gamma_i(x)$ (@eq-em-esperanza-indicador), que es lo único que se necesita de $Q(\Theta, \Theta^{(t)})$ (@eq-em-q-funcion) para continuar con el siguiente paso, es precisamente lo que hace el **Paso E**, de ahí su nombre, Expectativa. El **Paso M** completa el procedimiento encontrando $\Theta^{(t+1)} = \arg\max_\Theta Q(\Theta, \Theta^{(t)}),$ es decir, maximizando la esperanza calculada en el paso anterior; de ahí que se llame Maximización. A continuación se presentan ambos pasos de manera explícita.
